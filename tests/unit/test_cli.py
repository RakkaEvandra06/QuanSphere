...
class TestEncryptDecryptCli:
    ...
    def test_encrypt_with_password(self) -> None:
        # Password must be ≥ PASSWORD_MIN_LENGTH (12) characters.
        result = runner.invoke(app, ["encrypt", "secret", "--password", "mypassword_ok!"])
        assert result.exit_code == 0

    def test_encrypt_decrypt_with_password(self) -> None:
        # Password must be ≥ PASSWORD_MIN_LENGTH (12) characters.
        enc = runner.invoke(app, ["encrypt", "pbe test", "--password", "testpassword_ok!"])
        assert enc.exit_code == 0
        token = _extract_panel_value(enc.output)
        dec = runner.invoke(app, ["decrypt", token, "--password", "testpassword_ok!"])
        assert dec.exit_code == 0
...
def _extract_panel_value(output: str) -> str:
    """Extract and join all content from a Rich panel in CLI output."""
    ...
    for line in lines:
        ...
        if flat.startswith(("╰", "└")):
            ...
            continue
        if in_panel:
            # Strip │ from both sides (Rich adds one on each end), then trim
            # surrounding whitespace.  Skip any mid-panel horizontal rules
            # (contain the box-drawing character ─, U+2500).
            content = flat.strip("│").strip()
            if content and "\u2500" not in content:
                parts.append(content)

    if parts:
        # Join without separator: Rich wraps long values mid-token with no
        # continuation character, so concatenation reconstructs the original.
        return "".join(parts)

    # Fallback for output that does not use Rich panels (plain text).
    return next(
        (l.strip().strip("│").strip() for l in reversed(lines) if l.strip()),
        "",
    )